...
def get_artist(artist_id: int) -> Union[Artist, Tuple[ApiError, int]]:
    ...
    if not artist:
        return (ApiError(
            ...
            errors={"artist_id": f"No user found with id {artist_id}"}
        ), 404)
...
def post_artist() -> Union[PostArtist, Tuple[ApiError, int]]:
    ...
    if not user.stage_name:
        return (ApiError(
            message="Not an artist",
            errors={"artist": "You must upload a song first to become an artist"}
        ), 403)
    ...
    if "stage_name" in data and not isinstance(data["stage_name"], str):
        return (ApiError(
            ...
            errors={"stage_name": "Stage name must be a string"}
        ), 400)
    
    if "biography" in data and not isinstance(data["biography"], str):
        return (ApiError(
            message="Invalid biography",
            errors={"biography": "Biography must be a string"}
        ), 400)
