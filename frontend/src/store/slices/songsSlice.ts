import {
  ...
  export const fetchNewReleases = createAsyncThunk(
	...
	async (_, { dispatch }) => {
	  ...
	  // Add users to store
	  dispatch(userSlice.actions.addUsers(
		artists
		  ...
		  .map(artist => ({
			...
			first_release: artist.first_release ? new Date(artist.first_release) : undefined,
  ...
  export const selectSongsByArtist = createSelector(
	[(state: RootState) => state.song.songs, (_: RootState, artistId: UserId) => artistId],
	(songs, artistId): Song[] => 
	  Object.values(songs)
  ...
  export const selectSongById = (state: RootState, songId: SongId): Song | undefined =>
	state.song.songs[songId];
  
  export const selectSongComments = (state: RootState, songId: SongId): RSet<number> =>
	state.song.comments[songId] ?? {};
  ...
  const songsSlice = createSlice({
	...
	reducers: {
	  ...
	  addComment: (state, action: PayloadAction<{ songId: SongId; commentId: number }>) => {
	  ...
	  removeComment: (state, action: PayloadAction<{ songId: SongId; commentId: number }>) => {
		const { songId, commentId } = action.payload;
		if (state.comments[songId]) {
